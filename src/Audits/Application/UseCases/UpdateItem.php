...
final class UpdateItem
{
    ...
    public function __invoke(string $auditUuid, int $categoryId, int $itemId, array $data, User $actor): void
    {
        $audit = $this->repo->find(AuditId::from($auditUuid));
        if (!$audit) throw new DomainException('Audit not found');

        // Los administradores pueden actualizar items en cualquier local
